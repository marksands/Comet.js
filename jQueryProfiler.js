(function(){
  ...
  jQuery.displayProfile = function() {
		var str = "<div style='margin:0px auto;text-align:left;background:#FFF;color:#000;font-size:10px;width:800px;height:400px;overflow:scroll;padding:8px;margin:10px;'>";

    for ( var i = 0; i < log.length; i++ ) {
      var total = log[i].duration;
      str += "<big><b>Event: " + log[i].event + " (" + log[i].duration + "ms)</b></big><br/>" + "<table><tr><th>%</th><th>(ms)</th><th>Method</th><th>in</th><th>out</th></tr>";

      var methods = log[i].methods;
      for ( var m = 0; m < methods.length; m++ ) {
        var method = methods[m];
        str += "<tr><td>" + ((method.duration / total) * 100).toFixed(1) + "%</td><td>" + method.duration + "</td><td>" + (method.name == "jQuery" ? "" : "&nbsp;&nbsp;.") + method.name + "(" + method.args + ")</td>" + "<td>" + (method.inLength || "") + "</td><td>" + (method.outLength || "") + "</td></tr>";
      }

      str += "</table>";
    }

		jQuery.facebox( str + "</div>" );
  };
